...
def redistrict(DATA_DIR,steps=50,tree_loop=150,mino_coef=.5,buffer = 0,random=False,enrollment_err = 5/6, update_rate = 5,pictures='map'):
    ...
    block_data['MINO'] = block_data['HS_POP'] - (block_data['RRATIO']*block_data['HS_POP']).round()
    ...
    R = nx.to_networkx_graph(block_data['borders'].to_dict())
    ...
    # prettiness, colors, more colors, ect.
    if pictures == 'map':
      ...
      def _make_map(new_dist):
          bg = gpd.read_file(f'{DATA_DIR}/pa_groups /tl_2010_42_bg10.shp').set_index('GEOID10')
          bg['new_dist'] = 0
          for k,val in new_dist.items():
              for v in val:
                  bg['new_dist'].loc[v] = int(k)
          bg['new_dist'] = bg['new_dist'].astype('category')
          return bg.loc[list(R.nodes())].plot(column='new_dist', categorical=True, cmap=mymap)
        ...
        def _make_graph(new_dist):
          ...
          color_choice = ["#C0C0C0",'#808080','#FF0000','#800000','#FFFF00','#808000','#00FF00','#008000','#00FFFF','#008080','#0000FF','#000080','#800080']    
          ...
          for i,school in enumerate(new_dist):
              for u in new_dist[school]:
                  R.nodes[u]["color"]=color_choice[i%13]
              R.nodes[school]["color"]="#FF00FF" 
              R.nodes[school]["size"]=300
          display(nx.get_node_attributes(R,"color").values())
          return nx.draw(R,node_color=list(nx.get_node_attributes(R,"color").values()),pos=block_data['POS'].loc[list(R.nodes())],node_size=list(nx.get_node_attributes(R,'size').values()))
...
# sum columns of block_data to get total population
    ...
    def _calc_score(sch,district):
        ...
        tscore = sum([max([dist_dict[x][sch]-buffer,0])*block_data['HS_POP'].loc[x] for x in district])
    ...
    while wins < steps:
      ...
      # schs has length 2 it holds the two schools in the district
        ...
        if frames%update_rate == 0 and wins>0:
            stop = timeit.default_timer()
            print(f"{wins} --  Iteration Time: {np.round((frames/(stop-start)),2)} -- Diversity Score: {np.round(sum([district_scores[u][0] for u in schoollist])/tot_pop,3)} -- Mean Travel Time (sec): {np.round(sum([district_scores[u][1] for u in schoollist])/tot_pop,3)}")
    if pictures == 'map':
    ...
    if not random:
      final_diversity = round(sum([district_scores[u][0] for u in schoollist])/tot_pop,2)
      final_travel = round(sum([district_scores[u][1] for u in schoollist])/tot_pop,2)
