...
def redistrict(DATA_DIR, steps=10000000, tree_loop=150, coefs = [2,1,1], buffer=0, random=False, enrollment_err=10/11, update_rate=5, multipliers=[126.5,1,300],pictures='none'):
    ...
    school_dict_r_orig = deepcopy(school_dict_r)
    ...
    for step in tqdm(range(10000)):
        ...
        if improved:
            ...
            if frames % update_rate == 0:
                ...
                change_score = 0
                for i in school_dict_r:
                    if school_dict_r[i] != school_dict_r_orig[i]:
                        change_score += block_data['HS_POP'][i]
                from pprint import pprint
                schooldicts.update(improvements,school_dict)
                print(improvments,change_score)
    print('|'+'*'*3+'||'+'*'*22+'||'+'*'*26+'||'+'*'*32+'|')
    print(
    ...
    for i in school_dict_r:
        if school_dict_r[i] != school_dict_r_orig[i]:
            change_score += block_data['HS_POP'].loc[i]
    print(change_score)    
    if pictures == 'map':
