...
def redistrict_random(DATA_DIR, samples=100, steps=50, buffer=0, enrollment_err=10/11):
    ...
    def _calc_score(sch, district):
        ...
        comp_score = 0
        all_adj = [list(G.neighbors(i)) for i in district]
        for j in all_adj:
            for i in j:
                if i not in district:
                        comp_score += 2
        comp_score=10*comp_score/len(schoollist)
        if dem_score != dem_score:
            dem_score = 
        return (dem_score, trav_score, comp_score)
    ...
    district_scores, dem_scores,trav_scores,comp_scores= [],[],[],[]
    for i in tqdm(range(samples)):
        ...
        comp_scores.append(
            np.round(sum([scoring[u][2] for u in schoollist]), 3))
        if dem_scores[-1] !=  dem_scores[-1]: import ipdb; ipdb.set_trace()
    return district_scores, dem_scores, trav_scores, comp_scores
def pickle_save(obj, path):
    with open(path, "wb") as f:
        return pickle.dump(obj, f)
if __name__ == "__main__":
    results = redistrict_random('/Users/zoeshleifer/promys2020/lab/Gerrymandering2-promys2020/gerry_tree', samples=1000, steps=50)
    pickle_save(results,'comp_trav_test.pkl')